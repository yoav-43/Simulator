...
typedef struct {
	uint32_t regs[NUM_REGISTERS]; // Array to hold the register values (32-bit unsigned integer).
...
*/
uint32_t get_register(const Registers *registers, int reg_index);
...
*/
void set_register(Registers *registers, int reg_index, uint32_t value);
